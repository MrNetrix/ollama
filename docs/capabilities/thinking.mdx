...
Thinking is disabled by default. Set the `think` field on chat or generate requests to enable it. Most models accept booleans (`true`/`false`) or levels (`low`, `medium`, `high`, `max`), where `max` requests the highest thinking level.
...
- Explicitly disable thinking: `ollama run deepseek-r1 --think=false "Summarize this article"`
